...
func export_mesh_library(params):
    ...
    for child in scene_root.get_children():
        ...
        if mesh_instance and mesh_instance.mesh:
            ...
            # Add collision shape if available (Shape + Transform pairs required)
            var collision_added = false
            for collision_child in child.get_children():
                if collision_child is CollisionShape3D and collision_child.shape:
                    mesh_library.set_item_shapes(item_id, [collision_child.shape, collision_child.transform])
                    if debug_mode:
            ...
            # set_item_preview requires Texture2D — never pass Mesh (type errors).
            # Headless CLI has no EditorInterface previews; leave preview empty.
