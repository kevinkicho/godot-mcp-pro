...
## Transport: file IPC (legacy) + TCP JSON-lines via utils/runtime_tcp_server.gd (preferred).
extends Node

const REQUEST_PATH := "user://mcp_game_request"
const RESPONSE_PATH := "user://mcp_game_response"
const RuntimeTcpServer := preload("res://addons/godot_mcp/utils/runtime_tcp_server.gd")
const RuntimeCapture := preload("res://addons/godot_mcp/utils/runtime_capture.gd")
...
# Frame capture state (one-shot multi-frame)
...
# Input recording / monitor / signals / move-to (unchanged)
...
# TCP + video/timeline capture helpers
var _runtime_tcp = null
var _pending_tcp: StreamPeerTCP = null
var _capture = null  # RuntimeCapture


func _ready() -> void:
	process_mode = Node.PROCESS_MODE_ALWAYS
	_runtime_tcp = RuntimeTcpServer.new()
	_runtime_tcp.command_request.connect(_on_runtime_tcp_command)
	_runtime_tcp.start()
	_capture = RuntimeCapture.new()
	_capture.setup(self)
...
func _on_runtime_tcp_command(peer: StreamPeerTCP, req_id: int, command: String, params: Dictionary) -> void:
	_pending_tcp = peer
...
func _process(_delta: float) -> void:
	...
	if _pending_command and not FileAccess.file_exists(REQUEST_PATH) and not FileAccess.file_exists(RESPONSE_PATH):
		# Still waiting on TCP response path is OK while busy
		if _runtime_tcp and _runtime_tcp.is_busy() and _pending_tcp != null:
			pass  # async TCP command still in flight
		else:
	...
	match _state:
		...
		State.VIDEO_RECORDING:
			if _capture:
				_capture.process_video(_delta)
			if FileAccess.file_exists(REQUEST_PATH):
				_handle_request()
		State.CAPTURING_TIMELINE:
			if _capture:
				_capture.process_timeline(_delta)
			if FileAccess.file_exists(REQUEST_PATH):
				_handle_request()
...
func _dispatch_command(command: String, params: Dictionary) -> void:
	...
	if not concurrent_ok:
		# Abort any in-progress operation; finalize video so meta.json is written
		if _capture and _capture.is_active():
			_capture.abort_to_idle()
		_state = State.IDLE
	_pending_command = true

	match command:
		"ping_runtime":
			var info: Dictionary = _runtime_tcp.info_dict() if _runtime_tcp else {}
			info["pong"] = true
			info["state"] = _state_name()
			info["transport"] = "tcp" if _pending_tcp else "file"
			_write_response(info)
		"get_runtime_info":
			var info2: Dictionary = _runtime_tcp.info_dict() if _runtime_tcp else {}
			info2["state"] = _state_name()
			_write_response(info2)
		"get_scene_tree":
		...
		"log_run_event":
			if _capture:
				_capture.log_event(params)
			else:
				_write_response({"error": "capture helper missing"})
		"get_run_events":
			if _capture:
				_capture.get_events(params)
			else:
				_write_response({"error": "capture helper missing"})
		"get_run_logs":
			if _capture:
				_capture.get_logs(params, _state_name())
			else:
				_write_response({"error": "capture helper missing"})
		"get_run_status":
		...
		"capture_timeline":
			if _capture:
				_capture.start_timeline(params)
			else:
				_write_response({"error": "capture helper missing"})
		"find_nodes":
...
func _write_response(data: Dictionary) -> void:
	_pending_command = false
	# TCP client (preferred) — RuntimeTcpServer owns queue drain
	if _pending_tcp != null and _runtime_tcp:
		_pending_tcp = null
		_runtime_tcp.complete(data)
		return
	_pending_tcp = null
	# File IPC fallback
	...
	if file:
		...
		file.close()
	if _runtime_tcp and not _runtime_tcp.is_busy():
		_runtime_tcp.poll()  # allow queue drain after file path
...
func _cmd_get_run_status(_params: Dictionary) -> void:
	...
	if _capture:
		var extra: Dictionary = _capture.status_extras()
		for k in extra:
			payload[k] = extra[k]
	_write_response(payload)
